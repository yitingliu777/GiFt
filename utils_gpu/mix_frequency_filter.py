...
class GiFt_GPU(object):
    ...
    def get_cell_position(self, k, cell_pos):
        ...
        edge_index = edge_index.t().to(device)
        edge_weight = torch.from_numpy(trainAdj.data).float().to(device)

        norm_adj = torch.sparse.FloatTensor(edge_index,edge_weight).to(device)
        for _ in range(k):
            start = time.time()
